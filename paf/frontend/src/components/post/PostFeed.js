...
export default function PostFeed(props) {
  ...
  async function likeHandeler(id){
    const response = await fetch(
      `http://localhost:8082/api/v1/user/posts/${id}/likes/${uid}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  async function getAllLikes(){
    const response = await fetch(
      `http://localhost:8082/api/v1/user/posts/${props?.data.id}/likes`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <Card sx={{ marginTop: "1em" }}>
      ...
      <CardActions disableSpacing>
        <IconButton aria-label="add to favorites" onClick={()=>likeHandeler(props.data.id)} >
          <ThumbUp />
        </IconButton>
        <p>{likeCount+' liked'}</p>
        <ExpandMore
      ...
      {/* Edit comment form */}
      <Dialog open={opene} onClose={handleClosede} sx={{ zIndex: 1000 }} maxWidth="lg" >
